...
def register():
    ...
    return flask.render_template('register.html', length = len(users), users = users)
...
def test():
    if flask.request.method == "POST":
        ...
        artist = artist_id(artist_name=get_spotify_artist(name))

        db.session.add(artist)
        db.session.commit()

    items = artist_id.query.all()
    name = []

    for item in items:
        name.append(item.artist_name)
